...
Так же есть модуль с функиями-генераторами, которые способны генерировать номер карты, фильтровать списки транзакций по валюте, и выдавать описание операций.
В процессе выполнения домашних заданий данный репозиторий будет пополняться. 

### upd:

Был добавлен декоратор для логгирования функций. Может засекать время выполнения функции, сохранять ошибки в файл и т.д.
...
1. Включите нужный вам модуль(masks, processing, widget, generators, decorators) в ваш python проект
